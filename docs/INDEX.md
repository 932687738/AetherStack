# AetherStack 文档索引

| 文档 | 说明 |
|------|------|
| [README.md](../README.md) | 项目主文档 |
| [STEP1-DESIGN.md](../STEP1-DESIGN.md) | Step 1 设计结论与迁移策略 |
| [AGENTS.md](../AGENTS.md) | AI 强制入口 |
| [ARCHITECTURE.md](../ARCHITECTURE.md) | 架构概览 |
| [LOCALPATH.md](../LOCALPATH.md) | Monorepo 模块路径 |
| [CLAUDE.md](../CLAUDE.md) | Claude Code 索引（自动生成） |

## 快速上手

| 文档 | 说明 |
|------|------|
| [REPOS.md](REPOS.md) | **关联仓库说明（ai / ai_react）** |
| [GETTING_STARTED.md](GETTING_STARTED.md) | 新人 onboarding |
| [SUPERpower.md](SUPERpower.md) | obra Superpowers 插件 + .aetherstack 手册 |

## 指南

| 文档 | 说明 |
|------|------|
| [guides/openspec.md](guides/openspec.md) | OpenSpec 工作流 |
| [guides/harness.md](guides/harness.md) | Harness 六阶段 |

## 应用模块（关联仓库）

| 文档 | 路径 |
|------|------|
| 后端 README | `D:\cache\workspace\ai\README.md` |
| 前端 README | `D:\cache\workspace\ai_react\README.md` |

## OpenSpec

| 文档 | 说明 |
|------|------|
| [openspec/references/aether-rules.md](../openspec/references/aether-rules.md) | 项目规则 |
| [openspec/references/directory-examples.md](../openspec/references/directory-examples.md) | 目录示例 |
| [openspec/references/integration-contracts.md](../openspec/references/integration-contracts.md) | API 契约 |
| [openspec/OPENSPEC-SYNC.md](../openspec/OPENSPEC-SYNC.md) | 框架同步记录 |

## CI/CD

| 文档 | 说明 |
|------|------|
| [.github/workflows/README.md](../.github/workflows/README.md) | GitHub Actions 开关 |

## AI 工作流配置

- obra Superpowers 插件：`/add-plugin superpowers`（Cursor）
- 项目规则：`.aetherstack/`（`make sync-config`）

## Harness

- 配置：`harness/harness.config.yaml`
- 计划：`harness/docs/PLANS.md`
- 设计：`harness/docs/DESIGN.md`

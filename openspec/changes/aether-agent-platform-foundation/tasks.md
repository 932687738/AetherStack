...
- [ ] 1.5 执行 `mvn -Dtest=ToolResultTest test` — **可验证**：BUILD SUCCESS
...
- [ ] 2.4 **AUTO-UT**：`MyBatisAgentRegistryRepositoryTest`（Testcontainers 或 H2）— 插入/唯一约束 — **可验证**：单测通过
- [ ] 2.5 执行 `mvn -Dtest=MyBatisAgentRegistryRepositoryTest test` — **可验证**：BUILD SUCCESS
...
- [ ] 3.4 **AUTO-UT**：`AgentRegistryApplicationServiceTest` — active/deprecated 过滤 — **可验证**：单测通过
- [ ] 3.5 执行 `mvn -Dtest=AgentRegistryApplicationServiceTest test` — **可验证**：BUILD SUCCESS
...
- [ ] 4.4 **AUTO-UT**：`AgentRegistryControllerTest`（MockMvc）— **可验证**：单测通过
- [ ] 4.5 执行 `mvn -Dtest=AgentRegistryControllerTest test` — **可验证**：BUILD SUCCESS
...
- [ ] 5.5 执行 `mvn -Dtest=NamedChatClientModelRouterTest,SpringAiLlmCompletionAdapterTest test` — **可验证**：BUILD SUCCESS
...
- [ ] 6.4 **AUTO-AI-UT**：`SuperAgentHubLlmRouterTest` — transfer 目标选择 — **可验证**：单测通过
- [ ] 6.5 执行 `mvn -Dtest=PlatformRouterFacadeTest,SuperAgentHubLlmRouterTest test` — **可验证**：BUILD SUCCESS
...
- [ ] 7.5 执行 `mvn -Dtest=PrepareSuperAgentChatNodeTest test` — **可验证**：BUILD SUCCESS
...
- [ ] 8.4a **AUTO-AI-UT**：`PlatformAgentAdapterTest` — Mock ReactAgent Flux — **可验证**：StepVerifier 通过
- [ ] 8.5 执行 `mvn -Dtest=PlatformAgentAdapterTest test` — **可验证**：BUILD SUCCESS
...
- [ ] 9.2 前端：无 UI 变更（UI-FUNC N/A）— **可验证**：N/A
...
- [ ] 9.5 执行 `mvn -Dtest=SuperAgentChatApplicationServiceTest,AgentHubControllerTest test` — **可验证**：BUILD SUCCESS
...
- [ ] 10.4a **AUTO-AI-UT**：`PlatformRouterFacadeStickyTest` — 同 domain 命中 sticky — **可验证**：单测通过
- [ ] 10.5 执行 `mvn -Dtest=PlatformRouterFacadeStickyTest test` — **可验证**：BUILD SUCCESS
...
- [ ] 11.5 执行 `check-spring-ai-tools.ps1`（RagPlatformTool 四段式）— **可验证**：exit 0
- [ ] 11.6 执行 `mvn -Dtest=RagPlatformToolTest test` — **可验证**：BUILD SUCCESS
...
- [ ] 12.2 前端：无 — **可验证**：N/A
- [ ] 12.3 测试任务（待 test-cases Reviewed 后补充）
...
- [ ] 13.2 前端：无 — **可验证**：N/A
- [ ] 13.3 测试任务（待 test-cases Reviewed 后补充）
- [ ] 13.4 **AUTO-UT**：`SuperAgentChatTraceIdTest` — MDC 设置/清理 — **可验证**：单测通过
...
- [ ] 14.1 后端：`mvn compile` + scoped 测试全通过 — **可验证**：§5–§13 相关 `*Test` BUILD SUCCESS
- [ ] 14.2 Harness：`make verify`（ai 仓库）— **可验证**：verify 通过或记录已知存量债务

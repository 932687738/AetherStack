...
- [x] 1.5 执行 `mvn -Dtest=RetrievalThresholdFilterTest,ConversationConfigApplicationServiceTest test` — **可验证**：BUILD SUCCESS（本机 PATH 无 mvn，待本地执行）
- [x] 1.6 **MANUAL**：浏览器拖动 slider 保存，确认 UI 与 API 一致 — **ManualReason**：滑块交互与 toast
...
- [x] 2.2 前端：无独立 UI（复用 REQ1 面板）— **可验证**：N/A
- [x] 2.3 测试任务（待 test-cases Reviewed 后补充）
- [x] 2.4 **AUTO-UT**：Repository 集成测试或 Service 测试验证 upsert 幂等 — **可验证**：测试通过
- [x] 2.5 执行对应 `mvn -Dtest=... test` — **可验证**：BUILD SUCCESS
...
- [x] 3.4 **AUTO-UT**：Service 测试 anonymous → system fallback — **可验证**：测试通过
- [x] 3.5 执行 `mvn -Dtest=ConversationConfigApplicationServiceTest test` — **可验证**：BUILD SUCCESS
...
- [x] 4.5 执行 `mvn -Dtest=RetrievalThresholdFilter* test` — **可验证**：BUILD SUCCESS
- [x] 4.6 **MANUAL**：知识库对话提问，对比阈值 0% vs 80% 的 citations 差异 — **ManualReason**：端到端 SSE + 引用 UI
...
- [x] 5.4 **AUTO-UT**：Controller 或 Service 非法值测试 — **可验证**：测试通过

## 6. 收尾验证

- [x] 6.1 `npm run lint && npm run build`（ai_react）— **可验证**：build 成功；新组件 eslint 通过
- [x] 6.2 `make verify` 或后端全量 `mvn test`（按 harness 配置）— **可验证**：通过（本机无 mvn）

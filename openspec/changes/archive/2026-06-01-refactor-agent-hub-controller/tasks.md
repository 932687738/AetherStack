...
- [x] 1.1 后端：确认 `AgentHubController.chatAgent` 路径 `POST /api/agent-hub/chat/agent`、`AgentHubChatRequest` 字段、`TEXT_EVENT_STREAM` 产出不变；补充 `@WebMvcTest` 或存量契约测试断言 Content-Type 与 200 — **可验证**：`mvn -Dtest=AgentHubControllerContractTest test` 通过（或等价 MockMvc 测试）
- [x] 1.2 前端：无变更 — **可验证**：N/A（`uiCraftMode: disabled`）
- [x] 1.3 测试任务（待 test-cases Reviewed 后补充）
- [x] 1.4 **AUTO-UT**：`AgentHubControllerContractTest` — 正常 body 返回 SSE；空 message 4xx — **可验证**：MockMvc 通过
- [x] 1.5 执行 `mvn -Dtest=AgentHubControllerContractTest test` — **可验证**：BUILD SUCCESS
- [x] 1.6 **MANUAL**：Nebula Desk 智能体模式发起对话，Network 面板确认 URL/body/SSE — **ManualReason**：端到端 UI 联调
...
- [x] 2.4 **AUTO-UT**：`AgentHubControllerTest` 断言 `chatAgent` 仅调用 `AgentChatApplicationService.streamAgentChat`（Mockito verify）— **可验证**：单测通过
- [x] 2.5 执行 `mvn -Dtest=AgentHubControllerTest test` — **可验证**：BUILD SUCCESS
...
- [x] 3.6 **MANUAL**：日志确认每轮 chat 仅一次 prep graph invoke — **ManualReason**：运行时观测
...
- [x] 4.5 执行 `mvn -Dtest=AgentHubRouterTest,AgentChatDomainServiceTest test` — **可验证**：BUILD SUCCESS
- [x] 4.6 **MANUAL**：分别发送客服/代码/通用问题，核对 SSE 路由摘要前缀 — **ManualReason**：真实 LLM 路由观感
...
- [x] 5.4a **AUTO-AI-UT**：`AgentChatStreamAdapterTest` — Mock ReactAgent.stream → Flux 适配 — **可验证**：StepVerifier 通过
- [x] 5.5 执行 `mvn -Dtest=AgentChatStreamAdapterTest test` — **可验证**：BUILD SUCCESS
- [x] 5.6 **MANUAL**：代码生成场景触发 FileTools — **ManualReason**：工具+LLM 联调
...
- [x] 6.4 **AUTO-UT**：`ToolRegistryTest` — 无 Skill Bean 时 chat 可见工具仍含 DateTimeTools（独立 @Component 扫描）— **可验证**：单测通过
- [x] 6.5 执行 `mvn -Dtest=ToolRegistryTest test` — **可验证**：BUILD SUCCESS
...
- [x] 7.4 **AUTO-UT**：脚本门禁纳入 `make verify` 或 CI 可选步骤 — **可验证**：`.aetherstack/scripts/check-spring-ai-tools.ps1` exit 0
- [x] 7.5 执行 `check-spring-ai-tools.ps1` — **可验证**：无 ERROR
...
- [x] 8.6 **MANUAL**：构造多工具 plan 提问，确认无全量工具 schema 注入 — **ManualReason**：需观察 LLM 请求日志
...
- [x] 9.4 **AUTO-UT**：架构 grep 测试或 ArchUnit（可选）— agents.web 不 import orchestrator 除 status 迁移完成 — **可验证**：compile + 可选规则测试
- [x] 9.5 执行全量 `mvn test`（ai 仓库 agents 模块）— **可验证**：无回归失败
...
- [x] 10.4 **MANUAL**：Code Review 文档 diff — **ManualReason**：人工可读性
...
- [x] 11.2 前端：无 — **可验证**：N/A
- [x] 11.3 测试任务（待 test-cases Reviewed 后补充）
- [x] 11.4 **AUTO-UT**：`AgentHubDocumentationServiceTest` — Mock registry/catalog 断言 DTO 映射 — **可验证**：单测通过
...
- [x] 11.6 **MANUAL**：`make verify`（AetherStack + ai backend）— **ManualReason**：全链路验证

---

## 12. 收尾

- [x] 12.1 可选：`application.yml` 增加 `agenthub.chat.use-legacy-subagent` 回滚开关（默认 false）— **可验证**：配置文档说明
- [x] 12.2 **cr backend**：按 `rules/superpowers.md` 发起后端代码审查 — **可验证**：CR checklist 完成

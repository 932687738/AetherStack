...
- [ ] 1.5 执行 `mvn -Dtest=CliInvokeControllerTest test` — **可验证**：BUILD SUCCESS（本机 PATH 无 mvn，待本地执行）
...
- [ ] 2.5 执行 `mvn -Dtest=ExternalAiCliToolsTest test` — **可验证**：BUILD SUCCESS（本机 PATH 无 mvn）
- [ ] 2.6 **MANUAL**：本地安装 CLI 后通过智能体对话触发工具 — **ManualReason**：真实 CLI + LLM 选工具
...
- [ ] 3.5 执行 `mvn -Dtest=ProcessExternalCliExecutorTest test` — **可验证**：BUILD SUCCESS（本机 PATH 无 mvn）
...
- [ ] 4.5 执行 `mvn -Dtest=CliInvokeApplicationServiceTest test` — **可验证**：BUILD SUCCESS（本机 PATH 无 mvn）
...
- [ ] 5.4 **AUTO-UT**：可选 Logback ListAppender 断言字段存在 — **可验证**：测试通过（本期由 5.1 代码审查覆盖，未单独加 Appender 测试）
...
- [ ] 7.1 `mvn test`（或 `make verify` 后端段）— **可验证**：BUILD SUCCESS（本机 PATH 无 mvn）
- [ ] 7.2 **MANUAL**：本机配置 `codex`/`claude` 路径，REST + Agent 各调用一次 — **ManualReason**：真实 CLI 与凭据

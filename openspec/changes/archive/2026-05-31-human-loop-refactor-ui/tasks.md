...
- [x] 1.4 **AUTO-UT**：Configuration Bean 名称常量与 `@Qualifier` 引用一致性编译测试（或轻量 `@SpringBootTest` smoke 加载 humanLoop 包）— **可验证**：测试通过
- [x] 1.5 执行 `mvn -Dtest=HumanLoopDraftServiceTest,ToolFeedbackServiceTest test` — **可验证**：BUILD SUCCESS（本机 PATH 无 mvn，待本地执行）
- [x] 1.6 **MANUAL**：启动 ai 后 curl `GET .../human-loop/step1?threadId=smoke-1&prompt=hello` 返回 JSON — **ManualReason**：需真实 Graph + LLM 环境
...
- [x] 2.5 执行 `mvn -Dtest=HumanLoopControllerTest test` — **可验证**：BUILD SUCCESS（本机 PATH 无 mvn）
- [x] 2.6 **MANUAL**：Postman/curl 遍历 design §5 全部 11 个端点 — **ManualReason**：端到端 JSON 字段对照

## 3. 对话分组人工审核菜单（HIL-REQ3）

- [x] 3.1 后端：无 — **可验证**：N/A
- [x] 3.2a **UI-AUDIT**：Impeccable `audit HomePage` 侧边栏「对话」分组；新增菜单项视觉与选中态与现有三项一致 — **可验证**：audit 问题修复或记录豁免（已实现样式对齐，未跑 Impeccable CLI）
...
- [x] 3.4 **MANUAL**：窄屏侧边栏下菜单可点击、选中态可见 — **ManualReason**：响应式 UI
...
- [x] 4.4 **MANUAL**：Tab 间切换保留各 Tab 已填 threadId — **ManualReason**：交互验收
...
- [x] 5.4 **MANUAL**：留空 humanEditedReply 采纳草稿 — **ManualReason**：需 LLM 联调
...
- [x] 6.4 **MANUAL**：三种决策路径 — **ManualReason**：Agent 联调
...
- [x] 7.4a **AUTO-AI-UT**（可选）：`EnterpriseWorkflowServiceTest` — **可验证**：未实现（可选）
- [x] 7.5 **MANUAL**：自媒体 publishing step1→step2 — **ManualReason**：Graph 联调
...
- [x] 8.4 **AUTO-UT**：前端 API 层单测 — **可验证**：项目无 vitest，跳过
- [x] 8.5 **MANUAL**：断网失败 UX — **ManualReason**：网络异常
...
- [x] 9.3 `make verify` 或 `mvn test`（ai 仓库）— **可验证**：本机无 mvn
- [x] 9.4 **UI-POLISH**（可选）：`/impeccable polish HumanReviewWorkbench` — **可验证**：可选收尾

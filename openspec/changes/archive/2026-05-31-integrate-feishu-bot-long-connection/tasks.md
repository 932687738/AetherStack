...
- [x] 3.1 后端（ai / feishu）：确认 SDK 重连行为并补充可观测日志  
  - **依赖**：2.1  
...
- [x] 4.4 单测：`FeishuBotMessageApplicationServiceTest`（AUTO-UT）  
  - **依赖**：4.1  
...
- [x] 5.1 后端（ai / feishu）：统一日志规范与安全审查  
  - **依赖**：2.1、4.1  
...
- [x] 5.4 单测：日志不含 Secret（AUTO-UT，可选）  
  - **依赖**：5.1  

...
4. 在 **ai_react** 仓库实现；验收 `harness lint` + `harness build`（存量 Vite 模块可用 `npm run lint` + `npm run build` 至迁移完成）
...
- 与 Impeccable 命令链一致：先 shape/craft 或 audit，再 `harness lint` + `harness build`（Harness Verify；存量 Vite 见 `frontend-umi-standards.md` §8）

# Harness Engineering 工作流状态机与子 Agent 调度规范
...
| REQUIREMENT_DONE | 调度 hev-analyzer | 分析功能点+接口 | — |
...
**工具**：Read, Grep, Glob, maven-source-viewer, Explore

**职责**：
1. 分解需求为原子条件（boolean 可判定）
2. 定位每个条件的数据源和接口
3. 验证接口可用性（compile-guide 三步法）
4. 输出条件-接口映射表 + 实现文件清单
...
遵循 references/tech-design-principles.md 的条件-接口映射法。
输出格式见 references/tech-design-template.md。
```
...
方案：{技术方案摘要}
涉及文件：{文件清单}
编码规范见 references/code-patterns.md。
反模式警告见 references/common-mistakes.md。
```
...
验证代码质量，变更文件：{文件清单}
执行三步验证：
1. Linter: mvn checkstyle:check spotbugs:check spotless:check
2. 编译: mvn compile -DskipTests
3. 单测: 为变更的 Service 编写单元测试 + mvn test
Linter 配置见 references/linter-config-guide.md。
单测模板见 references/test-templates.md。
...
| 场景 | 关键词 | 流程调整 |
|------|--------|---------|
| CRUD | "增加/查询/修改/删除/CRUD" | 简化技术分析，跳过条件-接口映射 |
...
1. 编译验证必须在 Linter 之后、单测之前
2. 单测只写 Service 层，不写 Controller 层集成测试
3. 修复只改问题清单中的问题，不做额外重构
4. 每次修复后必须重新走完整三步验证
5. 问题清单必须按优先级分类（P0 编译/Linter > P1 测试 > P2 建议）
